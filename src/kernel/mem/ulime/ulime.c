...
int ulime_proc_kill(ulime_t *ulime, u64 pid) {
    ...
    while (proc) {
        if (proc->pid == pid) {
            printf("Killing process %s (PID %lu)\n", proc->name, pid);
...
int ulime_proc_mmap(ulime_t *ulime, ulime_proc_t *proc) {
    ...
    if (!phys_stack_pos) {
        printf("Erorr: proc stack pos is not allocated\n");
        return 1;
    }
    ...
    if (!phys_heap_pos) {
        printf("Erorr: proc heap pos is not allocated\n");
        return 1;
    }
    ...
    printf("Mapped process %s memory:\n", proc->name);
...
void ulime_schedule(ulime_t *ulime) {
    ...
    if (ulime->ptr_proc_curr->state == PROC_READY) {
        ...
        printf("Scheduled process: %s (PID %lu)\n", 
...
void ulime_proc_list(ulime_t *ulime) {
    printf("Process List:\n");
    printf("PID\tState\tName\t\tEntry Point\n");
    printf("---\t-----\t----\t\t-----------\n");
...
int ulime_load_program(ulime_proc_t *proc, u8 *code, u64 code_size) {
    if (code_size > proc->heap_size) {
        printf("Program too large for process heap\n");
        return 1;
    }
    ...
    printf("Loaded program (%lu bytes) into process %s at 0x%lX\n",
           code_size, proc->name, proc->entry_point);
    return 0;
}

u64 sys_exit(ulime_proc_t *proc, u64 exit_code, u64 arg2, u64 arg3) {
    printf("Process %s (PID %lu) exiting with code %lu\n", 
